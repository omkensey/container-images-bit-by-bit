...
* The labs assume a recent version of a Red-Hat-family (RHEL, Fedora, Rocky, etc.) Linux distribution.  If you are not using such a distribution, you may need to adjust some commands, e.g. to use `apt` instead of `dnf` to do package installs, but you should still be able to complete all the labs with most other major distributions (Debian, Ubuntu, etc.)
  ...
  * The lab instructions for running commands assume Rocky Linux 10.
...
* The `terraform` directory contains a Terraform configuration suitable for using Terraform or OpenTofu to spin up an AWS instance pre-configured to work through the lab exercises.  You do not need to use this to create a lab system suitable for running the labs but it may make it easier to get started.  The default login user for the Rocky AMI is `rocky` and by default the config creates a new AWS keypair and writes the private key file in the `files` subdirectory in the Terraform config directory (the path to the private key will be one of the outputs).
...
Let's set up a directory with a minimal base system and a shell. (If you're using a lab system set up with the supplied Terraform code, this block has already been done.)
...
If a process can be confined to a subset of the underlying filesystem, this suggests that that subset could be rolled up into a tidy archive and distributed as a unit, and in fact this is one way you can build the image for a container.  Of course to do this, you want the size of the resulting archive to be as small as possible.  One way to do this is to include only the specific binaries you need and only the specific libraries they depend on.  Let's try installing the `bash` binary in a chroot, but this time we'll just copy the binary and work through the library dependencies to make it work.
...
It doesn't work -- what's going on?  Although the error is a little obscure, what's wrong is that the `bash` binary is linked to shared libraries that we haven't copied yet, so they can't be loaded by a process chrooted to this directory.  Fortunately the tools exist to show us which libraries we need:

```
ldd $(which bash)
```

The `ldd` command will tell you the libraries `bash` is linked to, and their paths.  Copy each one to the corresponding path in the `chroot-minimal` directory:
...
If you have some extra time in this lab, try doing the same with other services.
...
The fact that five of the six layers in the second image are shared with the first image allows the build to be faster and the storage consumed to be smaller.

Layering also allows faster image pulls.  Pull the official `nginx:1.29.5` image:
...
Now explore the area where `podman` extracts the image layers before it runs the container.
...
If you're feeling curious, run the lab Terraform config again (or for the first time, if you used your own lab system) with the argument `-var create_freebsd_instance=true` and then log in to the new instance as `ec2-user` using the SSH private key file listed in the output.  Once you're logged in, read the documentation on [FreeBSD jails](https://docs.freebsd.org/en/books/handbook/jails/) and try creating one to run `nginx`.
